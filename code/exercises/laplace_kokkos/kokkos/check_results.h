...
int check_results(DataContext&       context_serial,
		  DataContextKokkos& context_kokkos,
		  Params&            params)

{
  ...
  for( int iy = iy_start; iy < iy_end /*&& (result_correct == 1)*/; iy++) {
	...
	fprintf(stderr,"ERROR: A[%d][%d] = %f does not match %f (reference) --- abs(delta)=%f\n",
  ...
  if (error_count) printf("Found %d errors\n",error_count);
