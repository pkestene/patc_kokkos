...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
\begin{frame}
  \frametitle{Hands-On 0: Build kokkos}
  ...
  \item \textbf{1. Get Kokkos sources, development branch}
    ...
    \end{itemize}
  \item \textbf{2. How to build and use}
    \begin{enumerate}
    \item Kokkos by design has {\bf many different configurations possible} (hardware adaptability) $\Rightarrow$ better be compiled as part as the target application
    \item \textcolor{red}{\textbf{Regular standalone Makefile (installed Kokkos):}} use \texttt{generate\_makefile.bash}, then \texttt{make kokkoslib; make install}\\
    ...
    \item \textcolor{blue}{\textbf{Embedded Kokkos source files in your application}} and use \myhref{https://cmake.org/}{CMake}:
      \begin{enumerate}
      \item \textcolor{darkgreen}{\bf (recommended) standalone cmake + kokkos sources embedded in your application}
      \item cmake+Tribits: only when Kokkos is build inside \myhref{https://github.com/trilinos/Trilinos}{Trilinos}
      \end{enumerate}
    \end{enumerate}
  \end{itemize}
...
\begin{frame}
  ...
  {\bf About standalone Makefile and environment variables settings for building on multiple architectures}
